...
class WebApp extends Generic implements EndpointInterface
{
     ...
     */
    public function __construct($email, $password, $answer, $backupcode, $platform)
    {
        parent::__construct($email, $password, $answer, $backupcode, $platform);
    }
    ...
    public function connect($backupcode)
    {
        ...
        //New connect()
        $this->login($url, 0, $backupcode);
        $url = $this->getMainPage();
        $this
            ->login($url, 1, $backupcode)
            ->getNucleusId()
            ->getShards()
            ->getUserAccounts()
            ->getSessionId()
     ...
     * @return $this
     */
    private function login($url, $try = 1, $backupcode)
    {
        ...
        $data = $forge
            ...
            ->sendRequest();
        if($try == 0 ) {
            $postURL = $data['response']->getEffectiveUrl(); //get location url from above post
            $forge = $this->getForge($postURL, 'get');
            $forge->removeEndpointHeaders()->sendRequest();
            sleep(3); //pause and allow time for EA to email code
            //do a request here to get code from inbox of email
            //post code to url
            $forge = $this->getForge($postURL, 'post');
            $body = $forge
                ->addHeader('Content-Type', 'application/x-www-form-urlencoded')
                ->removeEndpointHeaders()
                ->setBody(array(
                    "twoFactorCode" => $backupcode, //code here
                    "_trustThisDevice" => "on",
                    "trustThisDevice" => "on",
                    "_eventId" => "submit"
                ))
                ->sendRequest();
            //follow location of the rest of the urls
            $forge = $this->getForge($body['response']->getEffectiveUrl(), 'get');
            $data = $forge->removeEndpointHeaders()->sendRequest();
            $forge = $this->getForge($body['response']->getEffectiveUrl(), 'get');
            $data = $forge->removeEndpointHeaders()->sendRequest();
            $forge = $this->getForge($body['response']->getEffectiveUrl(), 'get');
            $data = $forge->removeEndpointHeaders()->sendRequest();
    ...
    private function getNucleusId()
    {
        ...
        if (!preg_match("/var\ EASW_ID = '(\d*)';/", $body, $matches)) {
            throw new \Exception('Login failed. AT NEUCLEAUS');
        }
    ...
    private function getPhishing()
    {
        ...
        if (isset($json['code']) && $json['code'] == 459 && $json['string'] == 'Captcha Triggered') {
            // if proper handler
            echo "Captcha Triggered bitches.";
    ...
    private function validate()
    {
        ...
        // need to validate
            ...
            $json = $forge
                ...
                ->addHeader('Content-Type', 'application/x-www-form-urlencoded')
                ->setBody(array(
                    'answer' => "ed04e7de7106842af09a7c69bd72c4cc"
                ))
                ->getJson();
